import contextlib
with contextlib.redirect_stdout(None):
    import pygame 

from cleint import Network
import random 
import os
pygame.font.init()

# Constants 
PLAYER_RADIUS = 10 
START_VEL = 9 
BALL_RADIUS = 5
FPS = 60


WIDTH , HEIGHT = 1000 , 500
pygame.display.set_caption("Agar.io game using sockets !")

NAME_FONT = pygame.font.SysFont("ARIAL BLACK" , 20)
TIME_FONT = pygame.font.SysFont("ARIAL BLACK" , 30)
SCORE_FONT = pygame.font.SysFont("ARIAL BLACK" , 26)

COLORS = [(255,0,0), (255, 128, 0), (255,255,0), (128,255,0),(0,255,0),(0,255,128),(0,255,255),(0, 128, 255), (0,0,255), (0,0,255), (128,0,255),(255,0,255), (255,0,128),(128,128,128), (0,0,0)]

# DYNAMIC VARIABLES
players = {}
balls = []

def convert_time(t):
    """
    convert a time given in seconds to a time in minutes """

    if type(t) == str :
        return t 
    if int(t) < 60 :
        return str(t) + "s"
    else :
        minutes = str(t // 60)
        seconds = str(t % 60)

        if int(seconds) < 10 :
            seconds = "0" + seconds
        
        return minutes + ":" + seconds 

def redraw_window(players , balls , game_time , score):
    WIN.fill((255 , 255 , 255))

    # drawing the balls in the game 
    for ball in balls :
        pygame.draw.circle(WIN , ball[2] , (ball[0] , ball[1]) , BALL_RADIUS)

    # drawing the players in the list
    for player in sorted(players , key = lambda x :players[x]["score"]):
        p = players[player]
        pygame.draw.circle(WIN , p["color"] , (p["x"] , p["y"]) , PLAYER_RADIUS + round(p["score"]))
        text = NAME_FONT.render(p["name"] , 1, (0,0,0))
        WIN.blit(text , (p["x"] - text.get_width()//2 , p["y"] - text.get_height()))

    # drawing the scoreboard
    sort_players = list(reversed(sorted(players , key = lambda x : players[x]["score"])))
    title = TIME_FONT.render("Scoreboard" , 1, (0,0,0))
    start_y = 25 

    x = WIDTH - title.get_width()
    WIN.blit(title , (x , 10))

    ran = min(len(players) , 3)
    for count , i in enumerate(sort_players[:ran]):
        text = SCORE_FONT.render(str(count + 1) + ". " + str(players[i]["name"]) , 1, (0,0,0))
        WIN.blit(text , (x , start_y+ count * 20))
    
    # showing the time
    text = TIME_FONT.render("Time : " + convert_time(game_time) , 1, (0,0,0))
    WIN.blit(text , (10 , 10))

    # displaying the score 
    text = TIME_FONT.render("Score : " + str(round(score)) , 1 , (0,0,0))
    WIN.blit(text , (10 , 15 + text.get_height()))

def main(name):
    """
    THIS IS THE MAIN FUNCTION WHIH WOULD BE USED FOR RUNNING THE GAME !
    """

    global players , balls 

    server = Network()
    current_id = server.connect(name)
    balls , players , game_time = server.send("get")

    clock = pygame.time.Clock()

    run = True 
    while run :
        clock.tick(FPS)
        player = players[current_id]
        vel = START_VEL - round(player["score"]//14)

        if vel <= 1 :
            vel = 1
        
        # getkey presses
        keys = pygame.key.get_pressed()
        data = ""

        # movements for the players
        if keys[pygame.K_LEFT] or keys[pygame.K_a] :
            if player["x"] - vel - PLAYER_RADIUS - player["score"] >= 0 :
                player["x"] = player["x"] - vel 
        
        if keys[pygame.K_RIGHT] or keys[pygame.K_d]:
            if player["x"] + vel + PLAYER_RADIUS + player["score"] <= WIDTH:
                player["x"] = player["x"] + vel

        if keys[pygame.K_UP] or keys[pygame.K_w]:
            if player["y"] - vel - PLAYER_RADIUS - player["score"] >= 0:
                player["y"] = player["y"] - vel

        if keys[pygame.K_DOWN] or keys[pygame.K_s]:
            if player["y"] + vel + PLAYER_RADIUS + player["score"] <= HEIGHT:
                player["y"] = player["y"] + vel

        data = "move " + str(player["x"]) + " " + str(player["y"])

        # sending data to server and receive back all other players information
        balls , players , game_time = server.send(data)

        for event in pygame.event.get():
            if event.type == pygame.QUIT :
                run = False
                break 
            if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE :
                # end the program when the user hits the esacpe key 
                run = False 
                break

        redraw_window(players , balls , game_time  , player["score"])
        pygame.display.update()
    
    server.disconnect()
    pygame.quit()
    quit()


if __name__ == "__main__":
# getting user names
    while True :
        name = input('Please enter your name : ')

        if 0 < len(name) < 25 :
            break 
        else :
            print("Please enter the name within 0-20 characters range ......")

    # setup pygame window
    WIN = pygame.display.set_mode((WIDTH , HEIGHT))
    pygame.display.set_caption("Agar.io")
    
    main(name)